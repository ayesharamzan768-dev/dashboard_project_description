...
def generate_all_charts(df):
    ...
    # 1. Pie Chart (Distribution of cases by Province - dot notation used)
    fig1, ax1 = plt.subplots(figsize=(6, 4))
    prov_data = df.groupby('Province').Reported_Confirmed_Cases.sum()
    ...
    # 3. Line Chart (Trends over time - dict agg to avoid double square brackets)
    fig3, ax3 = plt.subplots(figsize=(6, 4))
    yearly = df.groupby('Year').agg({'Reported_Confirmed_Cases': 'sum', 'Estimated_Cases_WHO': 'sum'}).reset_index()
    sns.lineplot(data=yearly, x='Year', y='Estimated_Cases_WHO', marker='o', label='WHO Estimated', color='red', ax=fig3.gca())
    sns.lineplot(data=yearly, x='Year', y='Reported_Confirmed_Cases', marker='s', label='Reported Confirmed', color='navy', ax=fig3.gca())
    ax3.set_title("Malaria Cases Trajectory Over Time")
    ax3.set_ylabel("Number of Cases")
    charts['line'] = fig3

    # 4. Bar Chart (Compare deaths across provinces - dot notation)
    ...
    # 7. Heatmap (Correlation matrix of features - Tuples and.filter() avoid brackets completely)
    ...
    corr = df.filter(items=numeric_cols).corr()
