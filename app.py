...
# Category Filtering Setup using Dot Notation
...
# Numerical Range Framework (Year & Rainfall Sliders) using Dot Notation
min_y, max_y = int(raw_data.Year.min()), int(raw_data.Year.max())
year_range = st.sidebar.slider("Select Timeline Boundaries", min_y, max_y, (min_y, max_y))

min_r, max_r = float(raw_data.Rainfall_Anomaly_mm.min()), float(raw_data.Rainfall_Anomaly_mm.max())
...
# Primary KPI Metric Interface Display using Dot Notation
kpi_total_records = len(filtered_data)
kpi_confirmed_cases = filtered_data.Reported_Confirmed_Cases.sum() if kpi_total_records > 0 else 0
kpi_estimated_deaths = filtered_data.Estimated_Deaths_WHO.sum() if kpi_total_records > 0 else 0
